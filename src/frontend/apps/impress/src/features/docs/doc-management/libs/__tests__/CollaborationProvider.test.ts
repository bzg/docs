...
describe('CollaborationProvider', () => {
  ...
  test('onClose increases failure count and initiates long polling when threshold exceeded', () => {
    const dummyEvent = {} as any;
    provider.isWebsocketFailed = false;
    provider.websocketFailureCount = provider.websocketMaxFailureCount;
    const pollSyncSpy = jest.spyOn(provider, 'pollSync').mockResolvedValue();
    const initCollaborationSSESpy = jest.spyOn(
      provider as any,
      'initCollaborationSSE',
    );
    provider.onClose(dummyEvent);
    expect(provider.websocketFailureCount).toBe(
      provider.websocketMaxFailureCount + 1,
    );
    // If threshold exceeded, pollSync with force and initiating SSE should be triggered.
    expect(pollSyncSpy).toHaveBeenCalledWith(true);
    expect(initCollaborationSSESpy).toHaveBeenCalled();
  });
});
